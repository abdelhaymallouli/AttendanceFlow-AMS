            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-5 bg-gray-50/50 rounded-2xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                    <div class="flex items-center gap-5">
                        <div class="w-12 h-12 {{ $record->status == 'present' ? 'bg-green-100 text-green-600' : ($record->status == 'late' ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-600') }} rounded-xl flex items-center justify-center font-black shadow-sm">
                            <i data-lucide="{{ $record->status == 'present' ? 'check' : ($record->status == 'late' ? 'clock' : 'x') }}" class="w-6 h-6"></i>
                        </div>
                        <div>
                            <p class="text-sm font-black text-gray-800 uppercase italic">{{ $record->session->module->name }}</p>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">{{ \Carbon\Carbon::parse($record->date)->format('M d, Y') }} • {{ \Carbon\Carbon::parse($record->session->start_time)->format('H:i') }}</p>
                        </div>
                    </div>
                    <span class="text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border
                        {{ $record->status == 'present' ? 'bg-green-50 text-green-600 border-green-100' : ($record->status == 'late' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-red-50 text-red-600 border-red-100') }}">
                        {{ $record->status }}
                    </span>
                </div>
                @empty
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-4 bg-red-50/50 rounded-2xl border border-red-100/50">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 {{ $record->status == 'late' ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-600' }} rounded-xl flex items-center justify-center font-black shadow-sm">
                            <i data-lucide="{{ $record->status == 'late' ? 'clock' : 'x' }}" class="w-5 h-5"></i>
                        </div>
                        <div>
                            <p class="text-sm font-black text-gray-800 uppercase italic">{{ $record->session->module->name }}</p>
                            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">
                                {{ \Carbon\Carbon::parse($record->date)->format('M d, Y') }} • {{ \Carbon\Carbon::parse($record->session->start_time)->format('H:i') }} 
                                @if($record->session->duration_hours) • {{ $record->session->duration_hours }}h @endif
                            </p>
                        </div>
                    </div>
                    <span class="text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border
