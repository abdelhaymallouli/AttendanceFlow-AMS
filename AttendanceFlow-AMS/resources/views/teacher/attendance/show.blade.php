...
@section('page_title', 'Marquer les présences')
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-bold text-blue-900">
                    {{ $session->module->name }}
                    <span class="opacity-50">&middot;</span> Group {{ $session->group->name }}
                </p>
                <p class="text-xs text-blue-700 mt-0.5">
                    {{ \Carbon\Carbon::parse($session->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($session->end_time)->format('H:i') }}
                     <span class="opacity-50">&middot;</span> {{ $students->count() }} étudiants
                </p>
            </div>
        </div>
        ...
        <div class="mt-4 pt-4 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4">
            <div class="flex flex-wrap justify-center md:justify-start gap-4 text-sm bg-gray-50 px-4 py-2 rounded-lg border border-gray-100">
                <div class="flex items-center">
                    ...
                    <span class="text-gray-500 text-xs font-medium uppercase tracking-wider">Présents : <strong class="text-gray-800 ml-1 text-sm" x-text="stats.present"></strong></span>
                </div>
                <div class="flex items-center">
                    <span class="w-2.5 h-2.5 bg-red-500 rounded-full mr-2"></span>
                    <span class="text-gray-500 text-xs font-medium uppercase tracking-wider">Absents : <strong class="text-gray-800 ml-1 text-sm" x-text="stats.absent"></strong></span>
                </div>
                <div class="flex items-center">
                    <span class="w-2.5 h-2.5 bg-amber-500 rounded-full mr-2"></span>
                    <span class="text-gray-500 text-xs font-medium uppercase tracking-wider">Retards : <strong class="text-gray-800 ml-1 text-sm" x-text="stats.late"></strong></span>
                </div>
                <div class="flex items-center">
                    <span class="w-2.5 h-2.5 bg-gray-300 rounded-full mr-2"></span>
                    <span class="text-gray-500 text-xs font-medium uppercase tracking-wider">Non marqués : <strong class="text-gray-800 ml-1 text-sm" x-text="stats.unmarked"></strong></span>
                </div>
            </div>
    ...
    <form id="attendanceForm" action="{{ route('teacher.sessions.attendance.store', $session) }}" method="POST">
        ...
        <x-ui.section-card :overflow="true" padding="none">
            ...
            <div class="hidden md:grid grid-cols-12 gap-4 p-4 bg-gray-50 border-b border-gray-200 font-bold text-xs text-gray-500 uppercase tracking-wider">
                ...
                <div class="col-span-5 text-center">Statut</div>
            </div>
            ...
            @if($students->isEmpty())
                <x-ui.empty-state 
                    icon="users" 
                    title="Aucun étudiant trouvé dans ce groupe."
