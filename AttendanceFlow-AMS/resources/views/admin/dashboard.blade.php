...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
    ...
    @php 
        ...
        $present = \App\Models\AttendanceRecord::where('date', now()->toDateString())->whereIn('status', ['present', 'late'])->count();
        $absent = \App\Models\AttendanceRecord::where('date', now()->toDateString())->whereIn('status', ['absent_unexcused', 'absent_excused'])->count();
    ...
    <div class="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                <div class="w-10 h-10 {{ in_array($record->status, ['present', 'late']) ? 'bg-green-100' : ($record->status == 'absent_excused' ? 'bg-amber-100' : 'bg-red-100') }} rounded-full flex items-center justify-center flex-shrink-0">
                    <i data-lucide="{{ in_array($record->status, ['present', 'late']) ? 'check-circle' : ($record->status == 'absent_excused' ? 'file-text' : 'x-circle') }}" class="w-5 h-5 {{ in_array($record->status, ['present', 'late']) ? 'text-green-600' : ($record->status == 'absent_excused' ? 'text-amber-600' : 'text-red-600') }}"></i>
                </div>
