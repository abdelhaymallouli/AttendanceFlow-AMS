...
@section('page_title', 'Create New Session')
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden" x-data="sessionForm('', '')">
        <div class="p-6 border-b border-gray-200 bg-gray-50/50">
            <h3 class="text-lg font-semibold text-gray-800">New Session Configuration</h3>
            <p class="text-sm text-gray-500">Fill in the module, teacher, and schedule for the new session.</p>
        </div>
                ...
                <div>
                    ...
                    <select name="teacher_id" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-4 focus:ring-blue-100 focus:border-blue-600 transition-all outline-none bg-white">
                        ...
                        @foreach($teacherProfiles as $teacher)
                            <option value="{{ $teacher->id }}" {{ old('teacher_id') == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->name }} ({{ $teacher->specialty }})</option>
                        @endforeach
                    </select>
